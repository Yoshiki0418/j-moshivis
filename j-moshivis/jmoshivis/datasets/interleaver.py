...
import torch
from jmoshivis.conditioners.base import ConditionAttributes
...
def tokenize(
    ...
    try:
        ...
        nl_piece = nl_encoded[-1] if len(nl_encoded) > 0 else None
    except Exception:
        nl_piece = None
    ...
    if alpha is not None:
        tokens = tokenizer.encode(text_segments, enable_sampling=True, alpha=alpha, nbest_size=-1)
    else:
        tokens = tokenizer.encode(text_segments)

    # --- 改行トークンまたはスペース相当で結合 ---
    if nl_piece is not None:
        tokens = reduce(lambda a, b: [*a, nl_piece, *b], tokens)
    else:
        # 改行トークンが存在しない場合はスペースで繋ぐ
...
class Interleaver:
    ...
    def __init__(
        ...
        main_speaker_label: str = "ASSISTANT",
