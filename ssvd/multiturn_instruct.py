...
def get_base_setting() -> Tuple[str, str, str, str]:
    ...
    system_1 = (
        ...
        "あなたの役割は、画像に写っている内容について詳しく質問することです。\n"
        "まずは目立つ特徴（主要な物体やその関係）から始め、次に細かい特徴（背景、季節、時間帯など）について聞いてください。\n"
        "実際には、上に与えられた説明文を参考に質問を作りますが、その事実を相手に明かしてはいけません。\n"
        "説明文に基づいて答えられる質問だけをしてください。\n"
        "はい/いいえ で答えられる単純な質問は避けてください。\n"
        "また、答えが分かってしまうような誘導的な質問（〜ですよね？など）もしないでください。\n"
        "会話のテンポを重視し、説明を長くしない \n"
    )

    system_2 = (
        "あなたは画像を見ている人として、相手の質問に答える役割です。\n"
        "相手に分かりやすいように、事実に基づいた答えを簡潔に伝えます。\n"
        "必ず画像の説明文に基づいて答えてください。説明文にない事実を作ってはいけません。\n"
        "相手が誤解していたら、その誤りを指摘してください。\n"
        "自然な会話を心がけ、必要であれば追加の観察を述べて会話を広げてください。\n"
        "例えば「画像の雰囲気」を聞かれた場合は、色合いや表情など観察可能な要素を根拠に答えてください。\n"
        "最も重要なのは、新しい事実を勝手に作らないことです。\n"
        "解答はできる限り、簡潔にすること \n"
        "会話のテンポを重視し、説明を長くしない \n"
    )
